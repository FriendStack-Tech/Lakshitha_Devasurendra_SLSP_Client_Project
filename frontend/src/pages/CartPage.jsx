...
import { redirectToPayhere } from '../utils/payhereRedirect';
import toast from 'react-hot-toast';

// ✅ Single source of truth — matches backend SHIPPING_FEE constant
...
const CheckoutDropdown = ({ cart, onOrderConfirmed }) => {
  ...
  useEffect(() => {
    const fetchAddresses = async () => {
      try {
        ...
        setAddresses(all);
        setSelectedAddress(all.find(a => a.IsDefault) || all[0] || null);
      } catch {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      console.log('   TotalAmount :', order?.TotalAmount);  // Should be subtotal + 350
      console.log('   ShippingFee :', order?.ShippingFee);  // Should be 350
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      // ✅ Always use Mongo _id for the API call
      const orderId   = placedOrder._id;
      const displayId = placedOrder.OrderID || placedOrder.OrderNumber || orderId;

      localStorage.setItem('pendingOrderDisplayId', displayId);
      localStorage.setItem('pendingOrderId', orderId);

      console.log('💳 Initiating payment for orderId:', orderId);
      const params = await paymentService.initiatePayment(orderId);
      console.log('💳 Payment params received:', params);

      redirectToPayhere(params);
      // No setPaymentLoading(false) — page is navigating away
  ...
  // ✅ Derive totals from cart + constant — no hardcoded magic numbers in JSX
  ...
  return (
    ...
    >
      ...
      <div className="co-drop__progress">
        {['Address', 'Confirm', 'Payment'].map((label, i) => {
          const stepKeys  = ['address', 'confirm', 'done'];
          const isActive  = stepKeys.indexOf(step) >= i;
          const isDone    = stepKeys.indexOf(step) > i;
          return (
            <React.Fragment key={label}>
              {i > 0 && <div className="co-drop__step-line" />}
              <div className={`co-drop__step ${isActive ? 'active' : ''}`}>
          ...
          {loadingAddresses ? (
            ...
            <div className="co-drop__addr-list">
              {addresses.map(addr => (
                <label
                  key={addr._id}
                  className={`co-drop__addr-option ${selectedAddress?._id === addr._id ? 'selected' : ''}`}
                >
                  <input
                    type="radio"
      ...
      {step === 'confirm' && (
        <div className="co-drop__body">
          ...
          <div className="co-drop__items-preview">
            {cart.items.map(item => (
              <div key={item.ProductID} className="co-drop__preview-item">
                ...
                <span className="co-drop__preview-price">
          ...
          <div className="co-drop__totals">
            ...
            <div className="co-drop__total-row">
              <span>Shipping</span>
              {/* ✅ Uses constant, not hardcoded 350 */}
          ...
          <div className="co-drop__addr-recap">
            ...
            <button className="co-drop__change-btn" onClick={() => setStep('address')}>
              Change
            </button>
          </div>

          <div className="co-drop__payment-note">
            <FiCreditCard size={13} /> Payment via <strong>PayHere</strong>
          </div>

          <div className="co-drop__footer">
            <button className="co-drop__btn co-drop__btn--ghost" onClick={() => setStep('address')}>
              ← Back
            </button>
            <button
      ...
      {step === 'done' && (
        <div className="co-drop__body co-drop__body--done">
          ...
          <p className="co-drop__success-id">
            Order ID:{' '}
            <strong>
              {placedOrder?.OrderID || placedOrder?.OrderNumber || '—'}
            </strong>
          </p>
          {/* ✅ Show the actual total from DB so user sees what they'll be charged */}
          <p className="co-drop__success-id">
            Total:{' '}
            <strong>
              Rs. {(placedOrder?.TotalAmount ?? grandTotal).toLocaleString()}
            </strong>
          </p>
          <p className="co-drop__success-sub">
            Your order is confirmed. Complete payment below.
          </p>
          <button
          ...
          >
            {paymentLoading
              ? <><div className="co-drop__btn-spinner" /> Redirecting…</>
      ...
      <style>{`
        ...
        .co-drop__step {
          ...
          font-size: 0.75rem; font-weight: 600; color: #c0a080;
        ...
        .co-drop__step-dot {
          ...
          font-size: 0.68rem; font-weight: 800; color: #c0a080;
          transition: all 0.2s;
        }
        .co-drop__step.active .co-drop__step-dot {
          background: #C8872A; border-color: #C8872A; color: #fff;
        }
        .co-drop__step-line {
          flex: 1; height: 1.5px;
          background: rgba(200,135,42,0.2); margin: 0 8px;
        }
        ...
        .co-drop__heading {
          ...
          color: #1A1208; margin: 0 0 16px;
        }
        .co-drop__heading-icon { color: #C8872A; }
        .co-drop__loading {
          display: flex; align-items: center; gap: 10px;
          padding: 16px 0; color: #a08060; font-size: 0.85rem;
        }
        .co-drop__spinner {
        ...
        .co-drop__addr-option {
          ...
          background: #fdf9f4; position: relative;
        }
        ...
        .co-drop__addr-option.selected {
          border-color: #C8872A; background: #fff8ee;
        ...
        .co-drop__addr-badge {
          ...
          text-transform: uppercase; background: rgba(200,135,42,0.15);
          color: #A06820; padding: 2px 7px; border-radius: 50px;
        ...
        .co-drop__items-preview {
          ...
          padding: 12px; background: #fdf9f4;
          border-radius: 10px; border: 1px solid rgba(200,135,42,0.1);
        }
        ...
        .co-drop__preview-item {
          display: flex; align-items: center; gap: 8px;
          font-size: 0.82rem; color: #5a4030;
        }
        ...
        .co-drop__totals { display: flex; flex-direction: column; gap: 7px; margin-bottom: 14px; }
        .co-drop__total-row {
          display: flex; justify-content: space-between;
          font-size: 0.85rem; color: #8a7055;
        }
        .co-drop__total-row--grand {
          font-size: 1rem; font-weight: 800; color: #1A1208;
          padding-top: 10px; border-top: 1.5px solid rgba(200,135,42,0.15); margin-top: 4px;
        }
        .co-drop__total-row--grand span:last-child { color: #A06820; font-family: Georgia, serif; }
        .co-drop__addr-recap {
          display: flex; align-items: flex-start; gap: 7px;
          font-size: 0.78rem; color: #8a7055; background: #fdf9f4;
          padding: 10px 12px; border-radius: 10px;
          border: 1px solid rgba(200,135,42,0.1); margin-bottom: 10px;
        }
        ...
        .co-drop__change-btn {
          ...
          white-space: nowrap; font-family: sans-serif; text-decoration: underline;
        }
        .co-drop__payment-note {
          display: flex; align-items: center; gap: 6px;
          font-size: 0.78rem; color: #a08060; margin-bottom: 14px;
        }
        ...
        .co-drop__footer {
          ...
          border-top: 1px solid rgba(200,135,42,0.1); gap: 10px;
        }
        .co-drop__link { font-size: 0.8rem; font-weight: 700; color: #C8872A; text-decoration: none; }
        .co-drop__link:hover { text-decoration: underline; }
        .co-drop__btn {
          display: inline-flex; align-items: center; gap: 7px;
          padding: 10px 20px; border-radius: 50px;
          font-size: 0.85rem; font-weight: 700;
          cursor: pointer; border: none; font-family: sans-serif; transition: all 0.2s;
        }
        .co-drop__btn--primary {
          background: linear-gradient(135deg, #C8872A, #A06820); color: #fff;
          box-shadow: 0 4px 14px rgba(200,135,42,0.35); margin-left: auto;
        }
        ...
        .co-drop__btn--ghost {
          background: none; color: #a08060;
          border: 1.5px solid rgba(200,135,42,0.2);
        }
        .co-drop__btn--ghost:hover { background: #fdf5ea; color: #A06820; }
        .co-drop__btn--confirm {
          background: linear-gradient(135deg, #2e8b57, #1e6b3f); color: #fff;
          box-shadow: 0 4px 14px rgba(46,139,87,0.35); flex: 1;
        }
        ...
        .co-drop__btn--pay {
          ...
          box-shadow: 0 6px 22px rgba(200,135,42,0.4); font-size: 0.95rem;
        }
        .co-drop__btn--pay:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 8px 26px rgba(200,135,42,0.5); }
        .co-drop__btn--pay:disabled { opacity: 0.6; cursor: not-allowed; }
        ...
        .co-drop__success-icon {
          ...
          display: flex; align-items: center; justify-content: center;
          color: #fff; box-shadow: 0 6px 20px rgba(46,139,87,0.35); margin-bottom: 4px;
        }
        .co-drop__success-title { font-family: Georgia, serif; font-size: 1.25rem; color: #1A1208; margin: 0; }
...
const CartPage = () => {
  ...
  useEffect(() => {
    ...
    if (checkoutOpen) document.addEventListener('mousedown', handleClickOutside);
  ...
  // ✅ Derived from constant — no magic numbers
  const grandTotal = cart.totalAmount + SHIPPING_FEE;
  ...
  if (cart.items.length === 0 && !checkoutOpen) {
    return (
      <div className="ct-empty">
        ...
        >
          <div className="ct-empty__icon"><FiShoppingBag /></div>
          <h2>Your cart is empty</h2>
          <p>Looks like you haven't added any spices yet!</p>
          <Link to="/shop" className="ct-btn ct-btn--primary">Continue Shopping</Link>
          <Link to="/" className="ct-empty__home">← Back to Home</Link>
        </motion.div>
        <style>{`
          .ct-empty { min-height: 70vh; display: flex; align-items: center; justify-content: center; padding: 40px 24px; background: #f9f4ec; font-family: sans-serif; }
          .ct-empty__card { background: #fff; border-radius: 24px; padding: 60px 48px; text-align: center; box-shadow: 0 8px 40px rgba(26,18,8,0.08); border: 1px solid rgba(200,135,42,0.1); max-width: 400px; width: 100%; }
          .ct-empty__icon { width: 88px; height: 88px; margin: 0 auto 24px; background: linear-gradient(135deg, #fdf3e3, #f5e5cc); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 2.2rem; color: #C8872A; border: 2px dashed rgba(200,135,42,0.3); }
  ...
  return (
    <div className="ct">
      ...
      <style>{`
        .ct { --amber: #C8872A; --amber-d: #A06820; --amber-l: #F5A94A; --dark: #1A1208; min-height: 100vh; background: #f9f4ec; font-family: sans-serif; }
        .ct__container { max-width: 1200px; margin: 0 auto; padding: 0 24px; }
        .ct__header { background: linear-gradient(135deg, #1A1208 0%, #3D2B0F 100%); padding: 40px 0 32px; }
        .ct__header-inner { display: flex; align-items: flex-end; justify-content: space-between; gap: 16px; }
        .ct__eyebrow { display: block; font-size: 0.72rem; letter-spacing: 3px; text-transform: uppercase; color: var(--amber-l); font-weight: 600; margin-bottom: 6px; }
        .ct__header h1 { font-family: Georgia, serif; font-size: clamp(1.8rem, 4vw, 2.6rem); color: #fff; margin: 0; }
        .ct__item-count { background: rgba(200,135,42,0.2); border: 1px solid rgba(200,135,42,0.35); color: var(--amber-l); font-size: 0.82rem; font-weight: 700; padding: 6px 14px; border-radius: 50px; white-space: nowrap; }
        .ct__body { padding: 36px 24px 72px; }
        .ct__layout { display: grid; grid-template-columns: 1fr 360px; gap: 28px; align-items: start; }
        .ct__items-card { background: #fff; border-radius: 20px; overflow: hidden; border: 1px solid rgba(200,135,42,0.1); box-shadow: 0 4px 20px rgba(26,18,8,0.06); margin-bottom: 16px; }
        .ct__item { display: grid; grid-template-columns: 88px 1fr auto auto auto; gap: 18px; align-items: center; padding: 20px 24px; border-bottom: 1px solid rgba(200,135,42,0.08); transition: background 0.2s; }
        .ct__item:last-child { border-bottom: none; }
        .ct__item:hover { background: #fdf9f4; }
        .ct__item-img { width: 88px; height: 88px; border-radius: 14px; overflow: hidden; background: linear-gradient(135deg, #fdf3e3, #f5e5cc); flex-shrink: 0; position: relative; }
        .ct__item-img img { width: 100%; height: 100%; object-fit: cover; }
        .ct__item-img-fallback { display: none; position: absolute; inset: 0; align-items: center; justify-content: center; font-size: 1.8rem; }
        .ct__item-cat { font-size: 0.68rem; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; color: var(--amber); display: block; margin-bottom: 4px; }
        .ct__item-name { font-family: Georgia, serif; font-size: 1rem; color: var(--dark); margin: 0 0 5px; line-height: 1.3; }
        .ct__item-unit-price { font-size: 0.78rem; color: #a08060; }
        .ct__qty { display: flex; align-items: center; background: #f9f4ec; border: 1.5px solid rgba(200,135,42,0.2); border-radius: 50px; overflow: hidden; }
        .ct__qty-btn { width: 34px; height: 34px; display: flex; align-items: center; justify-content: center; background: none; border: none; cursor: pointer; color: #8a7055; font-size: 0.85rem; transition: all 0.2s; }
        .ct__qty-btn:hover { background: var(--amber); color: #fff; }
        .ct__qty-val { min-width: 32px; text-align: center; font-weight: 700; font-size: 0.9rem; color: var(--dark); }
        .ct__item-total { font-weight: 800; font-size: 1rem; color: var(--amber-d); font-family: Georgia, serif; white-space: nowrap; text-align: right; }
        .ct__remove { background: none; border: none; cursor: pointer; color: #c0a090; font-size: 1rem; padding: 8px; border-radius: 8px; transition: all 0.2s; display: flex; align-items: center; justify-content: center; }
        .ct__remove:hover { color: #e74c3c; background: rgba(231,76,60,0.08); }
        .ct__items-footer { display: flex; justify-content: space-between; align-items: center; padding: 4px 0 16px; }
        .ct__back-link { display: flex; align-items: center; gap: 7px; color: var(--amber-d); text-decoration: none; font-size: 0.88rem; font-weight: 600; transition: gap 0.2s; }
        .ct__back-link:hover { gap: 10px; }
        .ct__clear-btn { display: flex; align-items: center; gap: 6px; background: none; border: none; cursor: pointer; color: #c0a090; font-size: 0.85rem; font-weight: 600; transition: color 0.2s; font-family: sans-serif; }
        .ct__clear-btn:hover { color: #e74c3c; }
        .ct__trust { display: flex; background: #fff; border-radius: 16px; border: 1px solid rgba(200,135,42,0.1); overflow: hidden; box-shadow: 0 2px 10px rgba(26,18,8,0.04); }
        .ct__trust-item { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 16px 10px; font-size: 0.75rem; font-weight: 600; color: #8a7055; border-right: 1px solid rgba(200,135,42,0.08); text-align: center; }
        .ct__trust-item:last-child { border-right: none; }
        .ct__trust-icon { font-size: 1.1rem; color: var(--amber); }
        .ct__summary-card { background: #fff; border-radius: 20px; padding: 28px; border: 1px solid rgba(200,135,42,0.12); box-shadow: 0 4px 20px rgba(26,18,8,0.07); position: sticky; top: 88px; }
        .ct__summary-title { font-family: Georgia, serif; font-size: 1.3rem; color: var(--dark); margin-bottom: 22px; padding-bottom: 16px; border-bottom: 1px solid rgba(200,135,42,0.12); }
        .ct__summary-rows { margin-bottom: 20px; display: flex; flex-direction: column; gap: 12px; }
        .ct__summary-row { display: flex; justify-content: space-between; align-items: center; font-size: 0.9rem; color: #8a7055; }
        .ct__summary-row--total { font-size: 1.1rem; font-weight: 800; color: var(--dark); padding-top: 16px; border-top: 1.5px solid rgba(200,135,42,0.15); margin-top: 4px; }
        .ct__summary-row--total span:last-child { color: var(--amber-d); font-family: Georgia, serif; }
        .ct__coupon { display: flex; align-items: center; border: 1.5px solid rgba(200,135,42,0.2); border-radius: 12px; overflow: hidden; margin-bottom: 18px; background: #fdf8f0; }
        .ct__coupon-icon { color: var(--amber); margin: 0 10px; font-size: 0.95rem; flex-shrink: 0; }
        .ct__coupon-input { flex: 1; border: none; background: none; padding: 11px 4px; font-size: 0.88rem; color: var(--dark); outline: none; font-family: sans-serif; }
        .ct__coupon-input::placeholder { color: #b0956e; }
        .ct__coupon-btn { padding: 11px 16px; background: var(--amber); color: #fff; border: none; cursor: pointer; font-size: 0.82rem; font-weight: 700; transition: background 0.2s; font-family: sans-serif; }
        .ct__coupon-btn:hover { background: var(--amber-d); }
        .ct__checkout-btn { width: 100%; padding: 16px; background: linear-gradient(135deg, var(--amber), var(--amber-d)); color: #fff; border: none; border-radius: 14px; font-size: 1rem; font-weight: 800; cursor: pointer; box-shadow: 0 6px 22px rgba(200,135,42,0.4); font-family: sans-serif; letter-spacing: 0.3px; display: flex; align-items: center; justify-content: center; gap: 8px; transition: box-shadow 0.2s; }
        .ct__checkout-btn--open { background: linear-gradient(135deg, #8a6030, #6a4820); box-shadow: 0 4px 14px rgba(100,70,20,0.35); border-radius: 14px 14px 0 0; }
        .ct__checkout-chevron { transition: transform 0.25s ease; font-size: 1rem; }
        .ct__checkout-chevron--up { transform: rotate(180deg); }
        .ct__auth-note { text-align: center; font-size: 0.82rem; color: #a08060; margin-top: 14px; }
        .ct__auth-note a { color: var(--amber-d); font-weight: 700; text-decoration: none; }
        .ct__auth-note a:hover { text-decoration: underline; }
        .ct__summary-note { display: flex; align-items: center; justify-content: center; gap: 7px; font-size: 0.78rem; color: #b0956e; padding-top: 14px; border-top: 1px solid rgba(200,135,42,0.1); margin-top: 14px; }
        @media (max-width: 960px) { .ct__layout { grid-template-columns: 1fr; } .ct__summary-card { position: static; } }
